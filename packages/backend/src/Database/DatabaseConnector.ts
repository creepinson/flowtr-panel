...
class DatabaseConnector {
	...
	public connect(): Promise<Mongoose> {
		const maxTries = Configuration.getInstance().values
			.mongodb_connection_tries;
		let tries = 0;

		CilaLogger.log("Connecting to database");
		return new Promise((resolve: any, reject: any) => {
			const connect = () =>
				DatabaseConnector.tryConnection()
					...
					.catch(() => {
						if (++tries >= maxTries) {
							...
							CilaLogger.log(
								`Could not connect to database (${tries}/${maxTries}), retrying...`,
							);
							connect();
						}
					});
	...
	private static tryConnection(): Promise<Mongoose> {
		return mongoose.connect(
			Configuration.getInstance().values.mongodb_url,
			{
